    ...
    public class VNManager : DialogueViewBase
    {
		...
		void Awake () {
			...
			if ( useResourcesFolders ) {
				var allSpritesInResources = Resources.LoadAll<Sprite>("Illustration");
            ...
            runner.AddCommandHandler<string, string>("ChangeExpression", ChangeExpression);
        }
        ...
        // VNManager 내부 또는 별도의 클래스로 구현할 수 있습니다.
        public void ChangeExpression(string actorName, string expressionName)
        {
            if (!actors.ContainsKey(actorName))
            {
                Debug.LogErrorFormat("ChangeExpression: 캐릭터 [{0}]가 존재하지 않습니다.", actorName);
                return;
            }

            // 예시: 캐릭터별 표정 스프라이트 Dictionary (실제 구현 시에는 미리 세팅되어 있어야 함)
            // 예를 들어, VNActor 내부에 Dictionary<string, Sprite> expressions를 추가할 수도 있습니다.
            Sprite newExpressionSprite = FetchExpressionSprite(actorName, expressionName);
            if (newExpressionSprite == null)
            {
                Debug.LogWarningFormat("ChangeExpression: 캐릭터 [{0}]의 [{1}] 표정을 찾을 수 없습니다.", actorName, expressionName);
                return;
            }

            // 전환 효과 없이 바로 스프라이트 교체
            actors[actorName].actorImage.sprite = newExpressionSprite;

            // 또는 페이드 효과나 다른 전환 효과를 넣고 싶다면, 코루틴을 통해 스무스하게 변경할 수 있음.
        }

        // 예시: 캐릭터와 표현에 따라 적절한 Sprite를 반환하는 함수
        Sprite FetchExpressionSprite(string actorName, string expressionName)
        {
            // 예시로, Resources 폴더에서 "Eve_happy", "Eve_sad"와 같이 파일명이 구성되어 있다고 가정
            string assetName = actorName + "_" + expressionName;
            return FetchAsset<Sprite>(assetName);
        }
